...
新增RPD地区收入汇总、CPD地区收入汇总两张可见工作表。初始统计月份由GUI年份下拉和月份勾选输入，默认本机当月。可多选最多12个月；CLI可重复传入 `--report-month YYYY-MM`。每个选中月份分别生成RPD/CPD汇总，多月表名附加YYYY-MM；参数无效时在读取源文件前报错。生成后直接编辑各汇总页C4:G4合并区域中的年月（C4），两口径可独立调整，无需重新运行工具。
...
修改基表黄色人工字段后：Ctrl+Alt+F9完成公式重算，再选择“数据→全部刷新”。月份位于订未发到小计五列上方，黄色C4:G4支持当年12个月下拉，也可填写跨年YYYY-MM并确认提示。B5前期累计标题用公式引用C4；1月显示前期累计（无）。基表及汇总表均不设置工作表保护，用户可自行创建透视、排序和调整布局；黄色底色及批注区分人工输入。

## 实施结构与守恒

- services/regional_summary.py：纯投影及Decimal汇总，不依赖Excel。
- adapters/regional_pivot.py：源单元格公式、原生缓存、透视结构、初始显示与样式。
- 单月两口径共享一个隐藏源_summary_source及同一份缓存；多月时每个月使用独立_summary_YYYY_MM和独立cacheId；前40列通过公式引用基表（源中原地区列标为“原始地区部”，分组列名为“地区部”），4个辅助字段用于地区、汇总项目、基表行号及RPD/CPD口径。每张透视以原生报表筛选器固定默认口径。
...
- 初始值由Python计算且与缓存一致；saveData开启，refreshOnLoad关闭，避免未完成公式重算时抢先刷新。用户按重算→刷新顺序更新。每个月内共享同一个缓存对象，多个月按生成顺序分配唯一cacheId，与实际缓存关系保持一致；保存重读两轮保持两口径明细各自正确。
...
## v0.10.0 文件与界面兼容修复（Issue #42）

移除pivotTableDefinition中非法r:id；grand只出现在rowItems，不放入pivotField/items；按Microsoft字体子元素顺序输出。保存边界执行有限OOXML规范化，微软Open XML SDK作为独立必过门禁。

GUI按数据、月份、保存分组，年份可下拉，月份可勾选/本月/全年/清空；后台线程只执行计算并通过队列返回，Tk更新只在主线程；防重复执行、运行状态、进度动画及打开结果/所在文件夹。小窗口滚动表单，底部操作固定可见。

此修复针对已复现的Microsoft SDK错误。SDK校验、LibreOffice和Windows程序验证都不能代替用户所用桌面Excel实机打开；用户原始问题文件/修复日志仍可用于复核。
