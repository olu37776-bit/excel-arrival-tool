...
- 实施分支：`main`
- 本次修正基线：`ed00ab713f143610ba6edef33cd84abc88312634`
- 修正规则：Issue #26
...
`收入预测`是系统计算金额；两个`手工调整收入预测`字段是人工金额。
...
必须能够读取并继承两个手工调整收入预测字段；上期`收入预测`不参与继承。
