...
- 实施分支：`main`
- 当前代码基线：`dc7b313ee9091004f2d2c740ba3a7b751025023c`
...
保持现有“合同号 + 履行供应中心”基表和五张业务 Sheet，只完成四项变化：

1. 新增三个手工金额字段：
   - `收入预测`
   - `手工调整收入预测（按RPD）`
   - `手工调整收入预测（按CPD）`
2. 多履行供应中心合同的`遗留量`和`当月新订货`只在“深供”行显示，其他供应中心行显示0，避免金额重复汇总。
3. 三个新增字段首次为空，后续从上一次结果继承。
4. 取消`备货总控标识`与`发货总控标识`不同步异常，即停止生成`CONTROL_FLAG_MISMATCH`。
...
- 原`是否手工调整收入月份 / 手工调整收入月份 / 调整备注`继续正常继承；
- 三个新增字段因为旧结果不存在对应列而保持空白；
- 缺少新增字段不得导致整个上一次结果不可用；
- RPD/CPD跨期比较继续按现有规则执行。

输入解析必须把三个新增人工金额字段按金额处理：
...
5. 不修改`contract_values`本身，避免影响同合同其他中心和业务计算。
