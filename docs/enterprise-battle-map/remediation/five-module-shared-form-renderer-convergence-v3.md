...
用户仅要求进展可见文案加“作战”，保留限定词；Create/Edit同一UI不代表ID/草稿/提交生命周期相同，按真实代码分别调查。当前只查底栏按钮样式和进展区域两块，后者须追踪主题/内容的API及实库映射；其他表单区域不动。双滚动用户确认可接受，不以滚动条数量判断共享失败。当前先按 `investigation/enterprise-form-progress-legacy-parity-survey-v2.md` 只读对照，不继承本文件历史实施写入权限。
