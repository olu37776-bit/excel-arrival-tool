...
独立Progress popup必须保留，但不能代替原表单内新增入口。用户已明确Create/Edit的进展UI相同：只读最新摘要、可展开新增按钮、主题/内容两个可编辑输入，均由共享特殊editor承载；不能退化成整个区域readonly。新建无历史时显示合法空摘要，不能删掉结构。
...
用户仅要求进展可见文案加“作战”，保留限定词；Create/Edit同一UI不代表ID/草稿/提交生命周期相同，按真实代码分别调查。当前按报告最小建议修复进展双写与三入口一致性；底栏按实际实施范围核验，其他表单区域不动。双滚动用户确认可接受，不以滚动条数量判断共享失败。已完成调查作为输入；修复后按 `reviews/enterprise-progress-single-source-independent-review-v1.md` 独立审查，不将本文件历史广范围写入权限扩展到本次定向修复。
