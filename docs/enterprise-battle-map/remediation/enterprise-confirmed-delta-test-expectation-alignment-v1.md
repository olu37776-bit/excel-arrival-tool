...
**状态：此前修复已报告 / 最新迁移Schema残留转专项V1定向修复**  
...
此前用户报告修复完成，最新又报告三个迁移/DB测试残留。当前按 `remediation/enterprise-migration-schema-test-alignment-v1.md` 定向修复；该任务只允许测试及直接helper/fixture和报告变更，不继承本文件通用流程中的生产修复写入范围。原独立结论保留，后续按联合复核V1第8节检查新HEAD。
...
上述数量是总业务身份数，不等于DB物理列数，也不等于表格/新增/编辑的可见或可编辑数量。物理列按具体迁移版本及持久化映射建立独立精确集合，不能直接拿业务总数代替。视图期望按已确认 visibility/mode 规则推导，并独立检查业务集合与规则是否正确。

## 5. 必须保留的验证能力

- 已有迁移测试所用旧 schema/旧 Excel fixture 必须保留原字段与必要数据（不要求用户当前根目录工作簿具备 Git 历史）；完整升级到最新版本时验证迁移前存在、终态删除及其他字段数据保留；单步/前缀历史测试则验证对应历史版本，不要求每一步都删除该字段。不能把旧输入也改成新 schema，让迁移测试退化为空操作。
...
当前三个测试及直接测试依赖按 `remediation/enterprise-migration-schema-test-alignment-v1.md` 执行。已执行历史SQL保持原样；历史schema预期可保留旧字段，真实完整生产链最终仍须符合最新结构。不能把所有历史字样判活动残留，也不能把真正终态残留改为合法预期。
最新专项WRITE_SCOPE优先：本次不改SQL、生产runner/迁移登记或真实DB。发现真实生产迁移缺口记录PRODUCTION_MIGRATION_GAP，保持有效失败并建议后续修复，不在“测试更新”名义下实施。
