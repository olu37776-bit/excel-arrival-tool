...
用户最新补充前轮复核因大量未提交改动无法固定HEAD。按 `integration/enterprise-review-snapshot-preparation-v1.md` 先恢复实际差异与本专项进度，已完成则直接准备提交/证据，未完成则继续本专项范围。完成后把本轮测试与其依赖的已授权实现纳入完整候选，不在脏工作树上反复交独立复核。
...
候选固定按 `integration/enterprise-review-snapshot-preparation-v1.md`：本地提交即可，不需push，Excel不入Git，受审实现不能依赖未提交代码。后续按联合复核V1第9节确认候选，再按第8节独立检查断言强度、迁移历史不变与真实终态；旧首页/Excel未受影响且有效的证据可复用，不因这次测试修复重做已完成业务。
