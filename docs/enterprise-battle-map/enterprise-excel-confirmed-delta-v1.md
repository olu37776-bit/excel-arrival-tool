...
本文件对以上两点优先于既有模块 Authority、Excel V0.2 字段冻结说明及历史固定字段数量断言。其余 V0.2 差异仍需逐项调查，不因本轮自动获准。旧报告保留为旧 HEAD 的证据。首页 V4 尚无新的独立核验结果，不据本轮任务改记为 PASS。
...
先以实际 Excel 和代码建立“整体空间”字段 → canonical key → DB 列 → 消费者映射。既有 TOB Authority 的该分类字段为 overallSpaceTier；其他模块逐一核实，不凭名称猜测。删除字段的当前定义及有效消费者，包括实际存在的表格、Create/Edit、校验、筛选、Heatmap、导入导出及 API 投影。最终不能通过隐藏 UI 或保留空选项冒充字段删除。

- 最终字段集合为现有已确认集合减去这个字段；按真实模块契约更新数量、顺序和断言，历史字段数不再是本轮目标。
