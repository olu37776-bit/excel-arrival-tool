...
历史调查曾确认以下双写形态；用户最新报告除MOX外四模块仍存在，当前准确状态以本地报告及新候选证据为准：
...
- Create/Edit中的特殊Progress editor采用同一进展UI结构：只读最新摘要、可展开新增按钮、主题和内容两个可编辑输入；不能把整个区域降级为readonly或用独立popup替代表单内编辑。新建无历史时展示合法空摘要；
- 从进展历史关系读取，不在任何模块业务表维护第二份可写文本事实。
...
本地应用主分支MOX/TOB的Create/Edit是参照。用户已明确两者进展UI结构相同；此前差异前提撤销。Create尚无父记录ID与Edit已有ID的草稿/提交生命周期仍须分别按真实代码查清，不能因UI相同而抹平保存语义。
本次修复/审查围绕报告定位的进展双写及三入口数据/交互；底栏按报告实际实施范围核验，不扩大为整体表单改造。双滚动用户确认可接受，不以数量直接判错。
共享机制必须承载这些既有能力，History单一存储约束不限制只能有一个UI入口；也不授权复制旧模块整套模板或恢复双写。精确接线和旧提交/取消时机以已完成的本地调查报告为证据；当前修复后按 `reviews/enterprise-progress-single-source-independent-review-v1.md` 独立核实，审查者不代实施。
...
- 添加/编辑进展后读取一致；
- Create/Edit均验证只读最新摘要、展开新增按钮、主题/内容可编辑控件；不能只测readonly摘要或仅测独立popup；
- 核验主题/内容的UI→API→物理表列/关系映射与latest读取，分别覆盖Create/Edit的真实提交/取消语义；
...
以下为最初建设顺序，保留为历史依据。当前任务、阻塞与已授权增量统一以 `authority-index.md` 为准；首页 V4 和 Excel 已确认两项变更均已获授权，不受旧先后顺序阻塞。共享机制继续按本架构执行。
