...
**状态：CURRENT REVIEW AUTHORITY / 既有复核基线保留；当前先执行表单双滚动/进展差异只读调查，后续复核按固定候选进行**  
...
## 12. 新增表单滚动与作战进展反馈：先调查

用户要求先按 `investigation/enterprise-form-scroll-progress-survey-v1.md` 调查，不在本联合复核会话现场修复。调查报告为docs/enterprise/investigations/enterprise-form-scroll-progress-survey-v1-report.md，它记录现象、根因证据与建议，不替代本文件的独立结论。
本次调查可读取带未提交改动的真实运行现场，须标注BASE_HEAD、相关diff/hash、服务来源和证据稳定性；不适用“必须先提交才能开始调查”的前置，也不能把工作树观察归为BASE_HEAD独立PASS。
调查结束后再根据真实发现确定修复范围；只读调查本身不改CSS/组件/测试/SQL/数据，不提交，不自动切换实施。未来修复完成后的独立核验仍须固定包含修复的新候选，按实际影响读取本调查和对应修复证据，保留旧报告的历史归属。
