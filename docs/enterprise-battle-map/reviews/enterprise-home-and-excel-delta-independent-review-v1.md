...
**状态：CURRENT REVIEW AUTHORITY / 当前重新调查底栏按钮、Create/Edit同一进展UI及数据库映射；只读不修复，双滚动不再作为问题**  
...
## 12. 主分支进展/底栏/样式对照：调查口径已纠正

当前入口为 `investigation/enterprise-form-progress-legacy-parity-survey-v2.md`，V1调查及旧本地报告保留历史。准确基线是D:\BattleMap\battle-map应用主分支MOX/TOB，不是云端文档仓库main或当前独立popup。
...
用户确认双滚动原本如此且可接受，不再因双滚本身阻塞。本次只有两块：底部固定取消/保存按钮样式、作战进展区域功能/样式。用户最新纠正：Create/Edit的进展UI相同，均为只读最新摘要、可展开新增按钮、主题/内容两个可编辑输入；撤销此前结构不同的前提，新增无历史不等于删除UI。两者的父记录ID/草稿/提交生命周期仍分别按真实代码核实。仅给UI“进展”文案加“作战”，保留限定词；其他表单区域及整体样式不动。
“表单readonly、独立弹窗有历史列表”可以是现状事实，但不是符合原行为的证明。History单一存储不等于UI仅只读，也不限制表单内与独立两个入口共用进展操作；不得要求为此恢复双写或改字段数。
重新调查使用docs/enterprise/investigations/enterprise-form-progress-legacy-parity-survey-v2-report.md，已有同名报告先归档，证据按BASE_HEAD/RUN_ID隔离。除两块UI对照外，必须记录主题/内容的model→API→实库表列/父记录关联、保存生命周期、latest读取和存储归属；区分实库观察、源码追踪与保存往返未运行，不能只写“使用History”。旧错误判断在新报告纠正，不改写旧客观证据。
本轮仍只读调查，允许记录未提交现场，不以先commit为前置，不修改代码/CSS/测试/SQL/业务数据或自动实施。后续恢复实现的独立核验另在固定新候选进行，须分别验证Create/Edit共同的四段进展结构、两者真实保存/取消/读回流程和固定底栏按钮样式，不能仅验证独立popup或字段名。
