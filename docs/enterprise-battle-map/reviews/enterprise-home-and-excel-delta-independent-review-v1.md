...
**状态：CURRENT REVIEW AUTHORITY / 当前仅调查底栏按钮、作战进展两块，按主分支分别保留新增/编辑差异；双滚动不再作为问题**  
...
用户确认双滚动原本如此且可接受，不再因双滚本身阻塞。本次只有两块：底部固定取消/保存按钮样式、作战进展区域功能/样式。按主分支同模块同模式对照，新增与编辑原本不同，不能硬套同一交互；原模式具备的摘要/按钮/主题内容展开能力须保留。仅给UI“进展”文案加“作战”，保留限定词；其他表单区域及整体样式不动。
“表单readonly、独立弹窗有历史列表”可以是现状事实，但不是符合原行为的证明。History单一存储不等于UI仅只读，也不限制表单内与独立两个入口共用进展操作；不得要求为此恢复双写或改字段数。
新调查报告为docs/enterprise/investigations/enterprise-form-progress-legacy-parity-survey-v2-report.md，记录主分支版本、当前diff、旧新能力/样式对照和最小恢复范围；纠正V1中不成立的等价判断，不改写其当时客观证据。
本轮仍只读调查，允许记录未提交现场，不以先commit为前置，不修改代码/CSS/测试/SQL/业务数据或自动实施。后续恢复实现的独立核验另在固定新候选进行，须按对应模式实测原有进展交互和固定底栏按钮样式，不强制新增/编辑一致，也不能仅验证独立popup或字段名。
