...
当前已有两项正式确认：删除“整体空间（肥肉/瘦肉/骨头）”分类字段；大企表名增加“交通”。执行规则见 `../enterprise-excel-confirmed-delta-v1.md`，无需再次等待 Authority Review。本调查仍只读原件，记录所有差异及输入版本；已授权修改在副本和对应项目任务中实施。本文件的只读限制不撤销上述实施授权，其他差异不得自动纳入。
