# DR-001 人工调整字段的初始化与继承

- 状态：`CONFIRMED`
- 日期：`2026-08-25`
- 关联：`docs/output-schema.md`、Issue #34

## 决策

最终基表中以下六个稳定字段ID属于人工维护字段：

1. `manual_revenue_segment`（显示为`调整收入分段类别`，任意普通Excel值）
2. `manual_adjust_flag`（显示为`是否手工调整预测`）
3. `manual_revenue_forecast_rpd`（显示为`调整月份（按RPD）`，`YYYY-MM`文本）
4. `manual_revenue_forecast_cpd`（显示为`调整月份（按CPD）`，`YYYY-MM`文本）
5. `manual_revenue_month`（显示为`调整金额`，Decimal两位小数）
6. `adjustment_note`（显示为`调整备注`）

旧字段ID `manual_revenue_segment_flag`和旧显示名仍用于上一次结果兼容读取。旧Y/N仅表示原人工标识，不能猜测为具体调整类别；旧列中的其他实际内容可以直接继承。新字段不做Y/N、类别枚举或类型限制，空白与数值0、布尔值等实际值必须区分。

`收入分段类别` 不属于人工维护字段，应按本次业务数据每次重新计算。`调整收入分段类别`只保存和展示用户自由填写的调整值，不反向修改该系统字段。

## 初始化

首次运行时，上述六个字段全部留空。

后续运行中新出现、且上一轮结果中不存在对应业务记录的行，上述六个字段同样留空。

## 后续运行继承

后续运行时，如果上一轮结果中存在同一业务记录，则继承上一轮的六个字段值，本次自动计算过程不得覆盖这些人工值。

稳定业务粒度为：

```text
合同号 + 履行供应中心
```

内部实现使用稳定字段 ID：

```text
contract_no + supply_center
```

输出显示名称变化不得破坏继承键。

## 与自动字段的边界

- `收入分段类别`：每次重新计算；
- `收入年月（按RPD）`：每次重新计算；
- `收入年月（按CPD）`：每次重新计算；
- 六个人工调整字段：首次为空，后续继承。

当前工具不根据人工调整字段反向修改 RPD、CPD、到货日期、自动收入年月或收入分段类别。

Issue #36明确最终分析视图：人工调整月份归一化后优先用于新增的最终收入年月，空白回退自动年月；最终分段/金额遵循对应人工实际值优先。四个最终字段不是人工字段，不继承，不依赖缓存。Excel中由实时公式更新；是否手工调整预测不门控任何最终值。它们仍不参与当前RPD/CPD跨月变化清单的比较，不覆盖自动值。完整字段、保护和实时月份范围见output-schema第1.1节。
